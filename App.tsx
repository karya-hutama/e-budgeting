...
// GANTI URL DI BAWAH INI dengan URL Web App Google Apps Script Anda jika tidak ingin menggunakan Env Var Vercel
const MASTER_BACKEND_URL = (import.meta as any).env?.VITE_BACKEND_URL || '';
...
const App: React.FC = () => {
  ...
  const [settings, setSettings] = useState<WebSettings>(() => {
    ...
    // Jika di kode ada MASTER URL tapi di localStorage kosong, gunakan MASTER URL
    if (!parsed.backendUrl && MASTER_BACKEND_URL) {
  ...
  const loadDataFromDatabase = useCallback(async () => {
    ...
    try {
      ...
      if (data) {
        // Update Global Settings dari DB jika tersedia
      ...
      showToast('Koneksi Database Gagal. Cek URL Backend.', 'error');
    } finally {
      setIsLoading(false);
    }
  ...
  // Sync Handlers (Sama seperti sebelumnya namun menggunakan backendUrl terkini)
  ...
  const handleSetSettings = async (newSettings: WebSettings) => {
    ...
    if (newSettings.backendUrl) {
      ...
      loadDataFromDatabase(); // Refresh data setelah ganti URL
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {isLoading && (
        <div className="fixed inset-0 bg-white/80 z-[200] flex flex-col items-center justify-center backdrop-blur-sm">
          <div className="w-12 h-12 border-4 border-[#f68b1f] border-t-transparent rounded-full animate-spin mb-4"></div>
          <p className="text-[#f68b1f] font-bold animate-pulse">Menghubungkan ke Database...</p>
        </div>
      )}
      {!currentUser ? (
